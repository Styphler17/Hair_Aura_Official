...
const Home: React.FC<HomeProps> = ({ onShopClick }) => {
  ...
  useEffect(() => {
    // Get top 3 products dynamically
    const allProducts = ProductService.getAll();
    setFeaturedProducts(allProducts.slice(0, 3));
  }, []);
  
  // Simulated TikTok Posts Data
  const tiktokPosts = [
  ...
  return (
    <div>
      {/* Hero Section */}
      <div className="relative bg-aura-black text-white h-[90vh] flex items-center overflow-hidden">
        ...
        <div className="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 w-full text-center md:text-left">
          <Reveal>
            <h2 className="text-sm md:text-base tracking-[0.3em] uppercase mb-6 text-neutral-300">
              Premium Virgin Hair in Ghana
            </h2>
          </Reveal>
          
          <Reveal delay={200}>
            <h1 className="text-5xl md:text-7xl lg:text-9xl font-serif font-bold mb-8 leading-[0.9]">
              Unleash Your <br />
              <span className="italic text-neutral-400">Inner Aura</span>
            </h1>
          </Reveal>

          <Reveal delay={400}>
            <p className="text-lg md:text-xl text-neutral-300 mb-10 max-w-xl font-light leading-relaxed">
              Experience the pinnacle of luxury with our ethically sourced, cuticle-aligned hair extensions. Served exclusively in Accra.
            </p>
          </Reveal>
          
          <Reveal delay={600}>
            <div className="flex flex-col md:flex-row gap-4 justify-center md:justify-start">
              <button onClick={onShopClick} className="group flex items-center justify-center gap-2 bg-white text-aura-black px-10 py-4 rounded-sm uppercase tracking-widest text-sm font-bold hover:bg-neutral-200 transition-all">
                Shop Now
                <ArrowRight size={16} className="group-hover:translate-x-1 transition-transform" />
              </button>
              <a href={`https://wa.me/${WHATSAPP_CONFIG.phoneNumber}`} target="_blank" rel="noopener noreferrer" className="flex items-center justify-center gap-2 border border-white text-white px-10 py-4 rounded-sm uppercase tracking-widest text-sm font-bold hover:bg-white hover:text-aura-black transition-all">
                WhatsApp Us
              </a>
            </div>
          </Reveal>
      ...
      <div className="bg-white py-20 text-center border-b border-neutral-100">
        <Reveal>
          <p className="text-xs font-bold uppercase tracking-[0.2em] text-neutral-400 mb-6">The Aura Standard</p>
          <div className="flex justify-center gap-8 md:gap-24 opacity-60 grayscale flex-wrap px-4">
            <span className="font-serif text-2xl font-bold text-neutral-800">100% VIRGIN</span>
            <span className="font-serif text-2xl font-bold text-neutral-800">HD LACE</span>
            <span className="font-serif text-2xl font-bold text-neutral-800">DOUBLE DRAWN</span>
          </div>
        </Reveal>
      </div>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <Reveal>
            <div className="text-center mb-16">
              <h2 className="text-4xl md:text-5xl font-serif font-bold text-aura-black mb-4">Best Sellers</h2>
              ...
              >
                  {tiktokPosts.map((post) => (
                      <a 
                          ...
                          className="group relative flex-none w-[280px] aspect-[9/16] bg-neutral-100 overflow-hidden snap-center cursor-pointer"
                      >
                          <img 
                              src={post.image} 
                              alt={post.desc} 
                          ...
                          <div className="absolute bottom-0 left-0 p-6 w-full text-white transform translate-y-2 group-hover:translate-y-0 transition-transform duration-300">
                              <div className="flex items-center gap-2 mb-2">
                                  <TikTokIcon size={12} />
                                  <span className="text-[10px] font-bold uppercase tracking-widest">{post.views} Views</span>
                              </div>
                              <p className="font-serif text-lg leading-tight line-clamp-2">{post.desc}</p>
                          </div>
                      </a>
                  ))}
              </div>
